...
#define FEATURE_ID_ICC_REGISTERS_READ       \
    X(ICC_IAR0_EL1		, 3, 0, 12, 8, 0)   \
    X(ICC_EOIR0_EL1		, 3, 0, 12, 8, 1)   \
    X(ICC_HPPIR0_EL1	, 3, 0, 12, 8, 2)   \
    X(ICC_BPR0_EL1		, 3, 0, 12, 8, 3)   \
    X(ICC_DIR_EL1		, 3, 0, 12, 11, 1)  \
    X(ICC_RPR_EL1		, 3, 0, 12, 11, 3)  \
    X(ICC_SGI1R_EL1		, 3, 0, 12, 11, 5)  \
    X(ICC_ASGI1R_EL1	, 3, 0, 12, 11, 6)  \
    X(ICC_SGI0R_EL1		, 3, 0, 12, 11, 7)  \
    X(ICC_IAR1_EL1		, 3, 0, 12, 12, 0)  \
    X(ICC_EOIR1_EL1		, 3, 0, 12, 12, 1)  \
    X(ICC_HPPIR1_EL1	, 3, 0, 12, 12, 2)  \
    X(ICC_BPR1_EL1		, 3, 0, 12, 12, 3)  \
    X(ICC_CTLR_EL1		, 3, 0, 12, 12, 4)  \
    X(ICC_SRE_EL1		, 3, 0, 12, 12, 5)  \
    X(ICC_IGRPEN0_EL1	, 3, 0, 12, 12, 6)  \
...
bool feature_id_is_icc_access(uintreg_t esr)
{
    ...
    return op0 == 3 && op1 == 0 && crn == 12 && crm >= 8 && crm <= 12;
}
